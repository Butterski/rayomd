...
- Added `scripts/stress_benchmark.py` to generate large mixed-style Markdown corpora and run single-file plus 10/100/1000-file batch benchmarks with max-RSS reporting.
...
Triggered by research notes in `optimization_research_notes.md`.

Accepted/practical result:

- Added `scripts/stress_ext4_benchmark.sh` to copy the project into native WSL storage, build there, and run the same generated stress corpus.
...
- Added `scripts/simdutf_large_unicode_ext4.sh` to compare `FAST_MARKDOWN_USE_SIMDUTF=OFF` vs `ON` on native WSL ext4.
...
- Added `scripts/commercial_benchmark.py` for deterministic synthetic corpora, 50+ test campaigns, JSON output, Markdown reports, and Linux/Windows memory reporting.
- Added `scripts/commercial_linux_ext4.sh` to run the Linux campaign on native WSL ext4 and copy metrics back without generated PDFs.
...
- Full tracked summary: `commercial_benchmark_summary.md`.
...
Generated by `python3 scripts/stress_benchmark.py --generate` with:
